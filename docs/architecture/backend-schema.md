...
| `plan_type`   | `VARCHAR(20)`  | NOT NULL, CHECK IN (`'weekly'`, `'monthly'`) | Enum-like constraint; extensible list per ADR-001 — add new values here, no migration of existing rows needed                                                                                                                                          |
