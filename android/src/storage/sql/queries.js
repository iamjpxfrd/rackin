// SQL implementation of the storage interface, generic over any driver
// satisfying { run(sql, params), all(sql, params), transaction(fn) }.
// Copied verbatim from server/src/storage/sql/queries.js — this file has no
// Node/browser/RN-specific code, only SQL strings and driver-shape logic, so
// there's nothing to change for the RN adapter (expoDriver.js) vs. the test
// driver (better-sqlite3) it was originally proven against. Duplicated
// rather than imported across a package boundary because android/ and
// server/ aren't wired into a shared workspace yet (open item in Task 3's
// "New" section); keep the two copies in sync by hand until that's resolved.
...
export function createSqlStore(driver) {
  ...
  return {
    ...
    // have told anything it happened.
