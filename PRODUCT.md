...
- New member registration + first payment in one flow, not two systems: name and plan type (`weekly` | `monthly`) required; amount and payment method (`cash` | `transfer`) recorded in the same action; member number auto-assigned sequentially (e.g. `"1001"`); QR code generated automatically; phone number optional, captured to support lapsed-member follow-up.
- Record payment for an existing member: extends `coversUntil` from the payment date + plan duration (weekly = 7 days, monthly = a flat 30 days, not a calendar month). Always calculated from the payment date, never extended from a prior `coversUntil`, even on early renewal — a deliberate pilot simplification.
...
- Explicitly out of scope for the pilot: member-facing app or login, multi-gym/multi-device support, online payment processing (RackIn records that a payment happened; it does not process one), automated billing reminders/auto-renewal, class scheduling/trainer booking, additional plan types beyond weekly/monthly.
