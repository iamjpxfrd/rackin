// HTTP transport for the sync push (TRD 5). Ported from
// server/src/sync/api.js — the only change is where the base URL/key come
// from: Vite inlines `import.meta.env.VITE_*` at build time, Expo inlines
// `process.env.EXPO_PUBLIC_*` the same way (any other prefix is stripped
// from the client bundle). RN's global `fetch` covers postJson() as-is, so
...
export function syncBaseUrl() {
  const configured = process.env.EXPO_PUBLIC_RACKIN_API_URL ?? "";
 ...
 *
 * Worth being clear about what this is: a key inside an app bundle is not a
 * secret from whoever holds the tablet — it closes the API to the internet,
 * not to the device's user. That is the honest limit of any credential a
 * client app can carry, and the reason real per-person authentication
 * belongs with the owner dashboard rather than here (ADR-002).
 */
export function syncApiKey() {
  const configured = process.env.EXPO_PUBLIC_RACKIN_API_KEY ?? "";
