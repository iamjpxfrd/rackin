...
- Check-in by numpad (primary, always available), QR scan (optional accelerant using the tablet's own camera, never a member's device), or name search — all three resolve to the same `checkInMember(memberId, method)` lookup. An expired/unpaid member is blocked at this lookup with a message telling staff to record a payment first; every other case (a fresh number, a same-day return) still goes through with no dead end.
...
- Needs-renewal list: members whose plan lapses within 7 days OR has already lapsed (fixed threshold), sorted worst-first — an already-expired member who's still visiting (so not yet on the lapsed list) still needs to appear somewhere.
- New member registration + first payment in one flow, not two systems: name, plan type (`session` | `weekly` | `monthly` | `annually`), and membership type (`regular` | `student`) required; amount and payment method (`cash` | `transfer`) recorded in the same action; member number auto-assigned as a random, non-sequential code (not guessable from registration order); QR code generated automatically; phone number optional, captured to support lapsed-member follow-up. The name field completes from names already on the roster — names repeat at one gym, and consistent spelling is what lets search find them later. Exact duplicate names are blocked unless staff explicitly confirm it's a different person.
...
- Record payment for an existing member: extends `coversUntil` by the plan duration (session = 1 day, weekly = 7 days, monthly = a flat 30 days not a calendar month, annually = 365 days), stacked on top of the member's remaining coverage — a renewal made before coverage lapses adds to it rather than resetting it. A lapsed member's renewal (or a first-ever payment) still starts from the payment date, since there's nothing left to stack onto.
...
- Explicitly out of scope for the pilot: member-facing app or login, multi-gym/multi-device support, online payment processing (RackIn records that a payment happened; it does not process one), automated billing reminders/auto-renewal, class scheduling/trainer booking, additional plan types beyond session/weekly/monthly/annually.
...
- Existing web client scaffold: Vite + React 19 + Tailwind CSS 4, with a local persistence layer already sketched at `server/db/db.js` (Dexie schema for `members`, `payments`, `checkIns`, deliberately matching the backend schema field-for-field).
